...
<ul> 
<li>Giá trị số cho website. Sử dụng bằng cách copy code ở cột bên phải và cho vào code (trong view sẽ bỏ đoạn <strong>this-></strong> đi).</li> 
<li>Hàm <strong>the_number()</strong> sẽ thực thi echo luôn. Nếu chỉ muốn lấy về giá trị mà không echo, hãy sử dụng: <strong>get_the_number()</strong></li> 
<li>Tham số của hàm <strong>the_number()</strong>: 
<ol> 
...
<li>Vào code tìm và xóa đoạn code tương ứng nếu không bản ghi sẽ tự động được tạo mới khi hàm <strong>get_the_number()</strong> hoặc <strong>the_number()</strong> được gọi trở lại.</li> 
</ol> 
</li> 
...
<li>Để thay đổi tên cho input, hãy điều chỉnh tham số: <strong>TRANS_NUMS_LABEL</strong></li> 
<li>Tìm và thay đổi code cho các phiên bản cũ: <strong>getconfig->custom_num_mon</strong></li> 
</ul> 
</div> 
...
<div class="lf f35"> 
<input type="text" class="span6" placeholder="<?php echo ConfigType::placeholder($k, $v); ?>" name="data[<?php echo $k; ?>]" id="data_<?php echo $k; ?>" value="<?php echo $v; ?>" onchange="return convert_option_number(this);" /> 
<?php 
